...
class Knearest:
    ...
    def majority(self, item_indices):
        ...
        if len(item_indices)>1:
            labels = numpy.take(self._y, item_indices)
            labels.sort()
            # return median(labels)
            if len(item_indices)%2==0:
                return int((labels[int((len(labels)-1)/2)] + labels[int(len(labels)/2)])/2)
            else:
                return labels[int((len(item_indices)-1)/2)]
    ...
    def classify(self, example):
        ...
        _,self._indexes = self._kdtree.query([example],self._k)
        self._flatIndexes = [subItem for item in self._indexes.tolist() for subItem in item]
        return self.majority(self._flatIndexes)
